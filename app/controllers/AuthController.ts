...
export class AuthController {
    ...
    async PostRegister(req: express.Request, res: express.Response) {
        ...
        if (!(await SetupService.GetEntity('enableRegistration')).boolValue) {
    ...
    async PostRequestAccess(req: express.Request, res: express.Response) {
        try {
            ...
            if (mailArr.length !== 2) {
                return res.status(400).send(ValidationResources.MailAddressInvalidFormat);
            }

            const domains = await ProxmoxService.getDomains();
            ...
            if (domains.find(x => x.domain === mailArr[1]) == null) {
                return res.status(400).send(ValidationResources.MailAddressNotManaged);
            }

            const secret = StringHelper.Generate(64);
            const auth = new Authentication();

            auth.email = email;
            auth.identifier = StringHelper.Generate(8);
