...
export class AuthService {
    ...
    async createSession(user: User, ip: string, pubKey: string): Promise<Session> {
        ...
        const sessionTimeout = (await SetupService.GetEntity('sessionTimeout')).numericValue;
        ...
        session.expired_at = DateHelper.addMinutes(sessionTimeout | 10);
...
StepOne e.V.`;
    ...
    async register(username: string, email: string, password: string) {
        ...
        try {
            if ((await SetupService.GetEntity('userRequireValidMail')).boolValue) {
